...
_YEAR = r"((?:19|20)\d{2})"
_RE_MONTH_YEAR_RANGE = re.compile(rf"{_MONTHS}\s*{_YEAR}\s*[-–—to]+\s*(?:{_MONTHS}\s*)?{_YEAR}|{_MONTHS}\s*{_YEAR}\s*[-–—to]+\s*(present|current|now)", re.IGNORECASE)
_RE_YEAR_RANGE = re.compile(rf"\b{_YEAR}\s*[-–—to]+\s*{_YEAR}|\b{_YEAR}\s*[-–—to]+\s*(present|current|now)", re.IGNORECASE)
...
def parse_resume(raw_text: str) -> dict[str, Any]:
    ...
    return {"skills": skills, "raw_sentences": raw_sentences, "total_years": total_years}
...
def _extract_total_years(text: str) -> float:
    ...
    for m in _RE_MONTH_YEAR_RANGE.finditer(text):
        ...
        years = []
        for g in groups:
            if g.lower() in ("present","current","now"): years.append(current_year)
            elif re.match(r"^\d{4}$", g): years.append(int(g))
        if len(years) >= 2: total += max(0, years[-1] - years[0])
    if total == 0.0:
    ...
    if total == 0.0:
        ...
        if direct_years: total = float(direct_years.group(1))
